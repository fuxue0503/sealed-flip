...
export default function App() {
  ...
  const [auctionKey, setAuctionKey] = useState(0); // Key to force reset of AuctionView
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f8fafc]">
      ...
      <header className="h-14 md:h-16 bg-[#0f172a] border-b border-white/5 sticky top-0 z-[60] flex flex-col shadow-2xl">
        <div className="max-w-7xl mx-auto h-full w-full px-4 flex items-center justify-between gap-8">
          ...
          <nav className="hidden lg:flex items-center gap-1 flex-1 max-w-lg bg-white/5 p-1 rounded-lg border border-white/5 overflow-x-auto no-scrollbar">
            ...
            <button onClick={() => { setCurrentTab('auction'); setAuctionKey(prev => prev + 1); }} className={`flex-1 min-w-[70px] py-1.5 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${currentTab === 'auction' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-white/40 hover:text-white hover:bg-white/5'}`}>Auctions</button>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 top-[104px] z-[50] bg-slate-900/95 backdrop-blur-xl animate-in slide-in-from-top-4 p-6">
          <div className="flex flex-col gap-3">
            {['home', 'market', 'terminal', 'launch_pad', 'auction', 'portfolio'].map(t => (
      ...
      <main className="flex-1 overflow-x-hidden">
        ...
        {currentTab === 'auction' && <AuctionView key={auctionKey} onBack={() => setCurrentTab('home')} />}
